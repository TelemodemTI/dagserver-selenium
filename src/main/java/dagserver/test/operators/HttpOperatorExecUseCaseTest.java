...
public class HttpOperatorExecUseCaseTest extends BaseTest {
	...
	public void createDagDesignWithStepTest(String url,String username,String pwd,String jarname,String dagname,String group,String cronexpr,String step1,ITestContext context) throws Exception {
    	...
    	if(login.login(username, pwd)) {
    		...
    		newform.addStep(dagname,step1,"main.cl.dagserver.infra.adapters.operators.HttpOperator");  
    		var params = newform.selectStage(step1);
    		var timeoutr = context.getCurrentXmlTest().getParameter("timeout");
    		var urlr = this.getInfrastructure(this.getClass().getCanonicalName(), "url");
    		var methodr = this.getInfrastructure(this.getClass().getCanonicalName(), "method");
    		var contentr = this.getInfrastructure(this.getClass().getCanonicalName(), "contentType");
