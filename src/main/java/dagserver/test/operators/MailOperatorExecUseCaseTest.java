...
public class MailOperatorExecUseCaseTest extends BaseTest {
	...
	public void createDagDesignWithStepTest(String url,String username,String pwd,String jarname,String dagname,String group,String cronexpr,String step1,ITestContext context) throws Exception {
    	...
    	if(login.login(username, pwd)) {
    		...
    		newform.addStep(dagname,step1,"main.cl.dagserver.infra.adapters.operators.MailOperator");    		
    		var body = context.getCurrentXmlTest().getParameter("body");
    		...
    		var userSmtp = this.getInfrastructure(this.getClass().getCanonicalName(), "userSmtp");
    		var pwdSmtp = this.getInfrastructure(this.getClass().getCanonicalName(), "pwdSmtp");
    		var fromMail = this.getInfrastructure(this.getClass().getCanonicalName(), "fromMail");
    		var toEmail = this.getInfrastructure(this.getClass().getCanonicalName(), "toEmail");
    		var subject = context.getCurrentXmlTest().getParameter("subject");
    		var params = newform.selectStage(step1);
    		params.sendParameter("host", host, "input");
    		params.sendParameter("port", port,"input");
    		params.sendParameter("userSmtp",userSmtp, "input" );
    		params.sendParameter("pwdSmtp", pwdSmtp,"input");
    		params.sendParameter("fromMail", fromMail,"input" );
    		params.sendParameter("toEmail", toEmail,"input" );
